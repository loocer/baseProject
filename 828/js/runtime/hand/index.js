...
import * as tools from '../../utils/tools'; // import Player from '../../player/index'

const databus = new DataBus(); // let player = new Player()
...
const leftHandCheck = (x, y) => {
  ...
  const deviation = 0;
  return !!(x >= thisx - deviation && y >= thisy - deviation && x <= thisx + common.HAND_WIDTH + deviation && y <= thisy + common.HAND_HEIGHT + deviation);
};
...
export const handEvent = () => {
  ...
  tt.onTouchStart(databus.touchHandler); // canvas.addEventListener('touchmove', ().bind(this))
  ...
  databus.moveHandler = e => {
    ...
    for (let p of e.touches) {
      ...
      if (lleft < lright) {
        ...
        if (l < 3600) {// this.isInsite = true
        } else {// this.isInsite = false
          }
      } else {
