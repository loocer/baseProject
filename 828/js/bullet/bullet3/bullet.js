...
import { groundWidth, groundHeight, GAME_IMG } from '../../utils/common';
...
export default class Bullet extends Sprite {
  ...
  init(x, y) {
    this.name = 'bullet3';
    this.zx = x;
    this.zy = y; // this.img = GAME_IMG.get('bullets')[0]

    this.x = x;
    this.y = y;
    this.showLength = 0;
    this.stopFlag = false;
    databus.createSpeed = 2;
  ...
  mucsic() {
    this.audio = tt.createInnerAudioContext(); // this.audio.autoplay = true
  ...
  drawToCanvas(ctx) {
    ...
    if (this.stopFlag) {
      ctx.drawImage(this.img, 0, this.height / 2 + this.showLength, 10, this.height, -this.width / 2, 0, this.width, length);
    } else {
      ctx.drawImage(this.img, 0, this.height / 2, 10, this.height, -this.width / 2, 0, this.width, length);
    }
  ...
  update() {
    ...
    if (this.y < 0 || this.y > groundHeight || this.x < 0 || this.x > groundWidth) {
      this.stopFlag = true;
    }
