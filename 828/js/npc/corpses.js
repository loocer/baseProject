...
import { biHuBody } from '../utils/common.js';
import { bleed1, bleed2, del1s2, bihu, duobi, GAME_IMG } from '../utils/common';
let dataBus = new DataBus();
export default class Corpses {
  constructor() {
    this.name = "corpses"; // console.log(del1s)
  }
  ...
  renderBody(ctx) {
    for (let i = 0; i < 4; i++) {
      ...
      let size = obj[4];
      ctx.drawImage(this.bodyPicS, 0, 0, obj[2], obj[3], x, y, size, size);
      ctx.restore();

      if (this.frame * 2 > 50) {
        let temp = [this.x, this.y, obj[0], null, 0, 0, this.Bodys[i][2], this.Bodys[i][3], x, y, size];
    ...
    if (this.frame * 2 > 50) {
      ...
      let obj = {
        time: 5,
        tempList: [this.x, this.y, this.rote]
      };
  ...
  renderBleed(ctx) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      ctx.drawImage(this.atlas, 0, 0, 100, 100, -size / 2, -size / 2, size, size);
      ctx.restore();
    }
  }
  ...
  update() {
    ...
    this.frame++; // console.log(this.del1s, ~~(this.frame / 10), '-------------', this.del1s[~~(this.frame / 10)])
