...
import { getRoteImg } from '../../utils/tools';
import { bleed1, bleed2, del1s2, bihu, duobi, GAME_IMG } from '../../utils/common'; // const ENEMY_WIDTH = 20
...
export default class Enemy extends Animation {
  ...
  init(x, y, li = 1) {
    this.x = x;
    this.y = y;
    this.name = 'enemy1'; // this.srcImg = srcImg

    this.imgs = GAME_IMG.get('black_bugs'); // this.img.src = imgSrc
  ...
  drawToCanvas(ctx) {
    if (!this.visible) return; // this.drawLive(ctx)

    let index = ~~this.frameSpeed;
    ctx.save();
    ctx.translate(this.x, this.y);
    ctx.rotate((this.rotate + 180) * Math.PI / 180);
    ctx.drawImage(this.imgs[index], -this.width / 2, -this.height / 2, this.width, this.height);
    ctx.restore();
  }
  ...
  getFrameTimeFlag() {
    ...
    return Math.abs(this.x - this.player.x) + Math.abs(this.y - this.player.y) > 10;
  }
  ...
  isCollideWith(sp) {
    ...
    return Math.sqrt((spX - tX) * (spX - tX) + (spY - tY) * (spY - tY)) < 20; // return !!(   spX >= this.x
