...
import { GAME_IMG } from '../../utils/common';
let databus = new DataBus();
const PLAYER_WIDTH = 60;
const PLAYER_HEIGHT = 60;
export default class Boom {
  constructor() {
    this.name = 'mosterHouse2';
  }
  ...
  drawToCanvas(ctx) {
    ...
    ctx.save();
    ctx.translate(this.x, this.y);
    ctx.drawImage(this.speedIcon, -this.width / 2, -this.height / 2, this.width, this.height);
    ctx.restore();
    this.drawlive(ctx);
  }
  ...
  update(player) {
    ...
    if (databus.frame % this.createTime == 0) {
      let enemy = databus.pools.getItemByClass('enemy2', Enemy);
