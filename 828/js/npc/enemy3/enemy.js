...
export default class Enemy extends Animation {
  ...
  init(x, y, li = rnd(2, 10)) {
    this.x = x;
    this.y = y;
    this.name = 'enemy3'; // this.srcImg = srcImg

    this.imgs = GAME_IMG.get('bihu_bugs'); // this.img.src = imgSrc
  ...
  getFrameTimeFlag() {
    return Math.abs(this.x - this.endX) + Math.abs(this.y - this.endY) > 50;
  }
  ...
  update(player) {
    ...
    if (!this.stop && !this.getFrameTimeFlag()) {
      this.stop = true; // console.log(33333)
