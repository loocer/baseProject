...
import { bulletsIcon } from '../utils/common';
...
export default class Boom {
  ...
  drawToCanvas(ctx) {
    ...
    ctx.drawImage(this.speedIcon, -this.width / 2, -this.height / 2, this.width, this.height); // ctx.fillStyle = '#000';
    // ctx.strokeStyle = '#000';
  ...
  update() {
    ...
    if (this.checkIsFingerOnAir()) {
      ...
      databus.bulletClass = this.bulletIndex; // if(this.bulletIndex==0){
      //   databus.audio.play(() => {
      //     console.log('开始播放')
      //   })
      // }else{
