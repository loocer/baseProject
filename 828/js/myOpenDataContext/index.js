let sharedCanvas = tt.getSharedCanvas();
let context = sharedCanvas.getContext('2d');
const screenWidth = tt.getSystemInfoSync().screenWidth;
const screenHeight = tt.getSystemInfoSync().screenHeight;
let sysInfo = tt.getSystemInfoSync(),
    width = sysInfo.windowWidth,
    height = sysInfo.windowHeight;
const ratio = tt.getSystemInfoSync().pixelRatio; // sharedCanvas.width = sharedCanvas.width * ratio;
// sharedCanvas.height = sharedCanvas.width * ratio;

let itemCanvas = tt.createCanvas();
...
function initRanklist() {
  ...
  for (let obj of list) {
    ...
    context.fillText(temp, tempx + 20, tempy + 50);
    context.drawImage(obj.atlas, 0, 0, 200, 200, tempx + 110, tempy, 100, 80);
    context.fillText(name, tempx + 250, tempy + 50);
    context.fillText(core, tempx + 500, tempy + 50);
    index++;
    temp++;
  }
} // 绘制自己的排名
...
function drawMyRank() {
  if (myInfo.avatarUrl && myScore) {
    ...
    context.fillText(myScore || 0, 630, 960 + 76); // 自己的名次
...
function sortByScore(data) {
  ...
  if (myRank === -1) myRank = array.length;
  return array;
...
function getFriendsRanking() {
  tt.getFriendCloudStorage({
    ...
    success: res => {
      let data = res.data;
      console.log(res.data); // drawRankList(data);
...
tt.onTouchMove(e => {
  ...
  startY = touch; // console.log('++++++++', moveY, touch, startY)

  initRanklist(); // // 触摸移动第一次触发的位置
  // if (startY === undefined) {
...
tt.onTouchEnd(e => {
  ...
  if (moveY < 0) {
    // 到顶
    moveY = 0;
  } else if (moveY > (list.length - 2) * 100) {
    // 到底
    moveY = (list.length - 2) * 100;
  }

  initRanklist(); // reDrawItem(moveY);
});
tt.onTouchStart(e => {// reDrawItem(moveY);
});
tt.onTouchStart(e => {
  let touch = e.touches[0];
  startY = touch.moveY; // startY = undefined;
  // if (moveY < 0) { // 到顶
  //   moveY = 0;
