...
import { GAME_IMG, playerBg } from '../utils/common';
...
export default class Player {
  ...
  drawToCanvas(ctx) {
    ...
    ctx.save();
    ctx.translate(this.x, this.y);
    ctx.rotate(this.rotateBody * Math.PI / 180); // ctx.drawImage(
    //   this.bg,
    //   -15,
    //   -15,
    //  30,
    ...
    ctx.drawImage(bu, -this.width / 2, -this.height / 2, this.width, this.height);
    ctx.restore();
    ctx.save();
    ctx.translate(this.x, this.y);
    ctx.rotate(this.rotateLag * Math.PI / 180 + 45.15);
    ctx.drawImage(this.bodyImg, -this.width / 2 + this.fireAcTime, -this.height / 2, this.width, this.height);
    ctx.restore();
    ...
    if (this.fireAcTime != 0) {
      ...
      ctx.drawImage(this.firePic, -5, -10, 10, 20);
      ctx.restore();
    }
  }
  /**
  ...
  isplesCollideWith(sp) {
    ...
    return Math.sqrt((spX - this.x) * (spX - this.x) + (spY - this.y) * (spY - this.y)) < sp.width / 3 + this.width / 3;
  }

  goOnLive() {
    databus.time = 2;
    databus.pageIndex = 2;
    tt.onTouchMove(databus.moveHandler);
    this.resetLife();
