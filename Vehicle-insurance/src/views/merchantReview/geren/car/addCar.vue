...
export default {
  ...
  methods: {
    ...
    onSubmit(){
      ...
      const vst = window.Object.assign({}, VehicleVO)
      const carList =[vst].concat(this.carList)
      this.$store.dispatch('ChangeCarList', carList)
      // this.$store.dispatch('addCartoList', VehicleVO)
      this.$emit('dialogMsg', {status:true})
      // const curruid = getUserId()
